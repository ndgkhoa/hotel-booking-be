...
const BookingDetailsController = {
    ...
    getTotalCostByHotelId: async (req: Request, res: Response) => {
        ...
        try {
            const bookingDetails = await BookingDetail.find({ hotelId }).populate('receiptId').lean()

            const totalRevenue = bookingDetails.reduce((total, detail) => total + detail.totalCost, 0)
